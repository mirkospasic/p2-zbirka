...
/* F-ja koja ispisuje sve permutacije od skupa {1,2,...,n} a[]
   ...
   se poziva sa argumentom m=1 jer formiranje
   permutacije pocinje od 1. pozicije. Stoga, nece se koristi a[0]. */
void permutacija(int a[], int m, int n)
{
  ...
  /* Izlaz iz rekurzije: Ako je pozicija na koju treba smestiti
     ...
     nalaze u permutaciji i ispisuje se permutacija. */
  if (m > n) {
    ispisiNiz(a, n);
    return;
  }

  /* Ideja: pronalazi se prvi broj koji moze da se postavi na
     ...
     Kada se to zavrsi, vrsi se provera da li postoji jos neki broj
     koji moze da se stavi na m-to mesto u nizu (to se radi u
     petlji). Ako ne postoji, funkcija zavrsava sa radom.
  ...
  for (i = 1; i <= n; i++) {
    /* Ako se broj i nije do sada pojavio u permutaciji od 1 do
       m-1 pozicije, onda se on postavlja na poziciju m i poziva se
       funkcija da napravi permutaciju za jedan vece duzine, tj.
       m+1. Inace, nastavlja se dalje, trazeci broj koji se nije

...
/* Repno-rekurzivna (eng. tail recursive)  je ona funkcija 
  Cije se telo zavrsava rekurzivnim pozivom, pri cemu 
  taj rekurzivni poziv ne ucestvuje u nekom izrazu.
  
  Kod ovih funkcija se po zavrsetku za tekuci rekurzivni poziv 
  umesto skoka na adresu povratka skace na adresu
  ...
  cime se smanjuje prostorna slozenost algoritma. 
 */

/* PomoCna funkcija koja izracunava n! * result. 
  Koristi repnu rekurziju. */

/* Result je argument u kom cemo akumulirati do tada izracunatu
  vrednost faktorijela.
  Kada zavrsimo, tj. kada dodjemo do izlaza iz rekurzije potrebno je 
  da vratimo result. */
...
/* Sada zelimo da se oslobodimo repne rekurzije koja postoji u 
  ...
  vratiti se na pocetak tela funkcije.
 */
...
/* Pisanje bezuslovnih skokova (goto naredbi) 
   nije dobra programerska praksa.
   Iskoristicemo prethodni medjukorak da bismo 
   dobili iterativno resenje bez bezuslovnih skokova.
 */
...
/* Nasim gore navedenim funkcijama pored n, mora da se salje
  i 1 za vrednost drugog argumenta u kome ce se akumulirati 
  rezultat. Funkcija faktorijel(n) je ovde radi udobnosti 
  korisnika, jer je sasvim prirodno da za faktorijel zahteva 
  samo 1 parametar. 
  Funkcija faktorijel izracunava n!, tako Sto odgovarajucoj gore 
  navedenoj funkciji koja zaista racuna faktorijel, 
  salje ispravne argumente i vraca rezultat koju joj ta funkcija 
  vrati. 
  Za testiranje, zameniti u telu funkcije faktorijel poziv 
  faktorijelRepna sa pozivom faktorijelRepna_v1, a zatim sa pozivom 

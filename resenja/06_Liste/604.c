...
int main()
{
    ...
    while ((pom = fgetc(in)) != EOF) {
        ...
        /* Ako je ucitana zatvorena zagrada proveravamo da stek 
           nije prazan i da li se na vrhu steka nalazi njegova
           odgovarajuca otvorena zagrada. */
            ...
            if (pom == ')' || pom == '}' || pom == ']') {
                if (stek != NULL
                    ...
                    /* Zagrade u aritmetickom izrazu nisu
                       ispravno uparene. */
                    break;
                }
            }
    ...
    if (stek == NULL && pom == EOF)
        ...
        while (stek != NULL) {
            /* Oslobadjamo memoriju koja je ostala na steku, u
               slucaju neispravnog uparivanja. */
            Cvor *tmp = stek->sledeci;
            free(stek);
            stek = tmp;
        }

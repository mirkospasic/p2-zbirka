...
void oslobodi_listu(Cvor ** adresa_glave)
{
  ...
  /* Nakon oslobodjenog repa, oslobadja se glava liste, i
     azurira se glava u pozivajucoj funkciji tako da odgovara
     praznoj listi */
  free(*adresa_glave);
  *adresa_glave = NULL;
}
...
/* Funkcija dodaje novi cvor na pocetak liste. Kreira novi cvor
   ...
   Funkcija vraca 1 ukoliko je doslo do greske pri alokaciji,
   inace vraca 0. */
...
/* Funkcija dodaje novi cvor na kraj liste. Prilikom dodavanja u 
   ...
   rep liste u rekurzivnom pozivu. U slucaju da je u rekurzivnom 
   pozivu doslo do greske pri alokaciji, funkcija vraca 1 visem
   rekurzivnom pozivu koji tu informaciju vraca u rekurzivni
   poziv iznad, sve dok se ne vrati u main. Ako je funkcija
   vratila 0, onda nije bilo greske. Tek je iz main funkcije
   moguce pristupiti pravom pocetku liste i osloboditi je celu,
   ako ima potrebe. */
int dodaj_na_kraj_liste(Cvor ** adresa_glave, int broj)
{
  if (*adresa_glave == NULL) {
    ...
    /* Azurira se vrednost na koju pokazuje adresa_glave i
       ujedno se azurira i pokazivacka promenljiva u pozivajucoj 
       funkciji. */
    *adresa_glave = novi;
    return 0;
  }
...
/* Funkcija dodaje broj u rastuce sortiranu listu tako da nova
   ...
   prosla bez greske, inace vraca 1 da bi ta vrednost bila
...
Cvor *pretrazi_listu(Cvor * glava, int broj)
{
  ...
  /* Ako nije nijedna od prethodnih situacija, pretraga se
...
{
  /* Trazenog broja nema ako je lista prazna ili ako je broj
     manji od vrednosti u glavi liste, jer je lista neopadajuce
     sortirana. */
...
void obrisi_cvor(Cvor ** adresa_glave, int broj)
{
  ...
  if ((*adresa_glave)->vrednost == broj) {
    ...
    /* Azurira se pokazivac na glavu da pokazuje na sledeci u
       listi i brise se cvor koji je bio glava liste. */

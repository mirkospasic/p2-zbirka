...
kraj]\n");
  ...
  /* Prethodno usluzivanje korisnika moze da se izvrsi i na
     sledeci nacin */
  /* while (skini_sa_reda(&pocetak, &kraj, &nov_zahtev)) {
     printf("\nSledeci je korisnik sa JMBG brojem: %s\n",
     nov_zahtev.jmbg); printf("sa zahtevom: %s\n",
     nov_zahtev.opis);
     ...
     if (strcmp(odgovor, "Da") == 0) dodaj_u_red(&pocetak,
     &kraj, &nov_zahtev); else fprintf(izlaz, "JMBG: %s\tZahtev: 
     %s\n", nov_zahtev.jmbg, nov_zahtev.opis);

     if (broj_usluzenih == VREME_ZA_PAUZU) { printf("\nDa li je
  ...
  /* Ukoliko je sluzbenik prekinuo sa radom, mozda je bilo jos
     neusluzenih korisnika, u tom slucaju treba osloboditi
     memoriju koju zauzima red sa neobradjenim zahtevima
     korisnika. */

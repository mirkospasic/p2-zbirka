...
int dodaj_sortirano(Cvor ** adresa_glave, int broj)
{
  ...
  /* U slucaju da je glava liste cvor sa vrednoscu manjom od
...
/* Funkcija trazi u listi cvor cija je vrednost jednaka datom
   broju. Vraca pokazivac na cvor liste u kome je sadrzan
   trazeni broj ili NULL u slucaju da takav cvor ne postoji u
   listi. Funkcija se u pretrazi oslanja na cinjenicu da je
   lista koja se pretrazuje neopadajuce sortirana. */
...
void obrisi_cvor(Cvor ** adresa_glave, int broj)
{
  ...
  while (*adresa_glave != NULL
    ...
    /* Adresu repa liste treba sacuvati pre oslobadjanja cvora
       na adresi adresa_glave. */
    pomocni = (*adresa_glave)->sledeci;
    free(*adresa_glave);
    *adresa_glave = pomocni;
  }

  /* Ako je nakon toga lista ostala prazna, izlazi se iz
     funkcije. */
  if (*adresa_glave == NULL)
    return;

  /* Od ovog trenutka, u svakoj iteraciji petlje tekuci pokazuje
     na cvor cija vrednost je razlicita od trazenog broja. Isto
     vazi i za sve cvorove levo od tekuceg. Poredi se vrednost
     sledeceg cvora (ako postoji) sa trazenim brojem. Cvor se
     brise ako je jednak, ili, ako je razlicit, prelazi se na
     sledeci cvor. Ovaj postupak se ponavlja dok se ne dodje do
     poslednjeg cvora. */
  tekuci = *adresa_glave;
  while (tekuci->sledeci != NULL)
    if (tekuci->sledeci->vrednost == broj) {
      /* tekuci->sledeci treba obrisati, zbog toga se njegova
         adresa prvo cuva u pomocni. */
      pomocni = tekuci->sledeci;
      /* Tekucem se preusmerava pokazivac sledeci, preskakanjem
         njegovog trenutnog sledeceg. Njegov novi sledeci ce
         biti sledeci od ovog cvor koji se brise. */
...
void obrisi_cvor_sortirane_liste(Cvor ** adresa_glave, int broj)
{
  ...
  /* Ako je nakon toga lista ostala prazna, funkcija se prekida. 
     ...
     veca od broja, jer kako je lista sortirana rastuce nema
  ...
  /* Od ovog trenutka se u svakoj iteraciji pokazivac tekuci
     pokazuje na cvor cija vrednost je manja od trazenog broja,
     kao i svim cvorovima levo od njega. Cvor se brise ako je
     jednak, ili, ako je razlicit, prelazi se na sledeci cvor.
     Ovaj postupak se ponavlja dok se ne dodje do poslednjeg
     cvora ili prvog cvora cija vrednost je veca od trazenog
     broja. */
  tekuci = *adresa_glave;
  while (tekuci->sledeci != NULL

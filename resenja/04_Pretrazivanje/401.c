 ...
 * na kojoj je pronadjen. Ovaj indeks je, kao sto znamo, uvek
 * nenegativan. Ako element nije pronadjen u nizu, funkcija
...
int binarna_pretraga(int a[], int n, int x)
{
    ...
    while (levi <= desni) {
        ...
        /*
         * Ako je srednji element veci od x, tada se x mora
         ...
         */
            ...
            /*
             * Ako je srednji element jednak x, tada smo
...
int interpolaciona_pretraga(int a[], int n, int x)
{
    ...
    while (levi <= desni) {
         ...
         * jednako ovim vrednostima, pa vracamo indeks levi
        ...
        /*
         * NAPOMENA: Indeks srednji je uvek izmedju levi i
         * desni, ali ce verovatno biti blize trazenoj vrednosti 
         * nego da smo prosto uvek uzimali srednji element. Ovo
         * se moze porediti sa pretragom recnika: ako neko trazi
         * rec na slovo 'B', sigurno nece da otvori recnik na
         * polovini, vec verovatno negde blize pocetku. 
         */
...
int main(int argc, char **argv)
{
     ...
     * novi seed za pozivanje random funkcije, i kako nas niz ne 
     * bi uvek isto izgledao seed smo postavili na tekuce vreme
     * u sekundama od Nove godine 1970. random()%100 daje
     * brojeve izmedju 0 i 99 
     */
    srandom(time(NULL));
    for (i = 0; i < n; i++)
        a[i] =
            i == 0 ? random() % 100 : a[i - 1] + random() % 100;
    ...
    /*
     * Racunamo novo vreme i razlika predstavlja vreme utroseno
     * za lin pretragu 
     */

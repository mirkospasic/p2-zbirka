...
unsigned int zamena(unsigned int x){
  ...
  /* Maska cetvrti bajt odgovara broju cija je binarna reprezentacija 
	 ...
	 a ostalo su nule) */
  maska_cetvrti_bajt = maska_prvi_bajt<<((sizeof(unsigned)-1)*BITOVA_U_BAJTU);
  ...
  prvi_bajt = (maska_prvi_bajt&x)<<((sizeof(unsigned)-1)*BITOVA_U_BAJTU);
  ...
  cetvrti_bajt = (maska_cetvrti_bajt&x)>>((sizeof(unsigned)-1)*BITOVA_U_BAJTU);

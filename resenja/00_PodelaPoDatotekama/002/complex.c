...
{
  /* Ispisuje se otvorena zagrada */ 
   printf("(");
  /* Zatim se razlikuju dva slucaja
     1) realni deo kompleksnog broja razlicit od nule: tada se realni
     deo ispisuje na standardni izlaz, nakon cega se ispisuje znak + 
	 ili - u zavisnosti da li je imaginarni deo pozitivan ili 
	 negativan, a potom i apsolutna vrednost imaginarnog dela 
	 kompleksnog broja
     2) realni deo kompleksnog broja je nula: tada se samo ispisuje
     imaginaran deo, s tim sto se ukoliko su oba dela nula ispisuje 
     samo 0, bez decimalnih mesta */
  if (z.real != 0) {
    printf("%.2f", z.real);

...
/* NAPOMENA:
   Zaglavlje math.h sadrzi deklaracije raznih matematickih 
   funkcija. Između ostalog, to su sledeće funkcije:
   double sin(double x);
   double cos(double x);
   double tan(double x);
...
/* Funkcija tabela() prihvata granice intervala a i b, broj 
   ekvidistantnih tačaka n, kao i pokazivač f koji pokazuje 
   na funkciju koja prihvata double argument, i vraća double 
   vrednost. Za tako datu funkciju ispisuje njene vrednosti 
   u intervalu [a,b] u n ekvidistantnih tačaka intervala */   
...
/* Umesto da koristimo stepenu funkciju iz zaglavlja 
   math.h -> pow(a,2) pozivaćemo korisničku sqr(a) */
double sqr (double a) 
{
    return a*a;
}
...
int main(int argc, char *argv[]) 
{
  double a, b;
  int n;
  /* Imena funkicja koja ćemo navoditi su kraća ili tačno duga
     5 karaktera */
  char ime_fje[6];
  ...
  /* Niska ime_fje sadrži ime tražene funkcije koja je navedena 
     u komandnoj liniji */
  ...
  /* Mreza mora da uključuje bar krajeve intervala, 

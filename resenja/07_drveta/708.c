...
void kopiraj_stablo(Cvor * koren, Cvor ** duplikat){
  /* Izlaz iz rekurzije: ako je stablo prazno nema sta da se kopira */
  if (koren == NULL){
    *duplikat = NULL;
    return;
  }
...
stabla  */
  *duplikat =napravi_cvor(koren->broj);
  ...
  adresu cuvamo u pokazivacu na desno podstablo korena duplikata. */
...
void kreiraj_presek(Cvor ** adresa_korena1, Cvor * koren2) {
  ...
  /* Kreiramo presek levog i desnog podstabla sa drugim stablom, tj.
iz  levog i desnog podstabla prvog stabla brisemo sve one elemente
koji ne  postoje u drugom stablu */
...
/* Funkcija izracunava razliku dva stabla - rezultujuce stablo se
dobija modifikacijom prvog stabla */
void kreiraj_razliku(Cvor ** adresa_korena1, Cvor * koren2) {
  /* Ako je prvo stablo prazno, tada je i rezultat prazno stablo */
  if (*adresa_korena1 == NULL)
    return ;
  /* Kreiramo razliku levog i desnog podstabla sa drugim stablom, tj.
iz  levog i desnog podstabla prvog stabla brisemo sve one elemente
...
bi  mogli da ga iskoristimo i za preostale operacije */
  ...
  /* Kreiramo presek stabala: prvo napravimo kopiju prvog stabla kako
  bi mogli da ga iskoristimo i za preostale operacije;
  */
  kopiraj_stablo(koren1,&pomocni);

...
\begin{enumerate}
\item Definisati strukturu \kckod{Cvor} kojom se predstavlja čvor liste. Čvor treba da sadrži ceo broj \kckod{vrednost} i pokazivač na sledeći čvor liste.
...
\item Napisati funkciju \kckod{Cvor *napravi\_cvor(int broj)} koja kao argument dobija ceo broj, kreira nov čvor liste, inicijalizuje mu polja i vraća njegovu adresu.
 ...
 \item Napisati funkciju \kckod{Cvor *pronadji\_mesto\_umetanja(Cvor * glava, int broj)} koja vraća pokazivač na čvor u neopadajuće uređenoj listi iza kojeg bi trebalo dodati nov čvor sa vrednošću \kckod{broj}.

 \item Napisati funkciju \kckod{int dodaj\_iza(Cvor * tekuci, int broj)} koja iza čvora \kckod{tekuci}. dodaje novi čvor sa vrednošću \kckod{broj}.
 ...
 \item Napisati funkciju \kckod{Cvor *pretrazi\_listu(Cvor * glava, int broj)} koja proverava da li se u listi nalazi čvor čija je vrednost jednaka argumentu \kckod{broj}. Vraća pokazivač na pronađeni čvor ili \kckod{NULL} ukoliko ga ne pronađe.

 \item Napisati funkciju \kckod{Cvor *pretrazi\_sortiranu\_listu(Cvor * glava, int broj)} koja proverava da li se u listi nalazi čvor sa vrednošću \kckod{broj}, pri čemu se pretpostavlja da se pretražuje neopadajuće uređena lista.

 \item Napisati funkciju \kckod{void obrisi\_cvor(Cvor ** adresa\_glave, int broj)} koja briše sve čvorove u listi koji imaju vrednost jednaku argumentu \kckod{broj}.

 \item Napisati funkciju \kckod{void obrisi\_cvor\_sortirane\_liste(Cvor ** adresa\_glave, int broj)} koja briše sve čvorove u listi koji imaju vrednost jednaku argumentu \kckod{broj}, pri čemu se pretpostavlja da se briše iz neopadajuće uređene liste.
...
\begin{upotreba}{1}
#\naslovInt#
#\izlaz{Unosite brojeve: (za kraj CTRL+D)}#
#\ulaz{2}#
#\izlaz{Lista: [2]}#
#\ulaz{3}#
#\izlaz{Lista: [3, 2]}#
...
\begin{upotreba}{2}
#\naslovInt#
#\izlaz{Unesite brojeve: (za kraj CTRL+D)}#
#\ulaz{23}#
#\izlaz{Lista: [23]}#
#\ulaz{14}#
#\izlaz{Lista: [23, 14]}#
...
#{Unesite broj koji se brise:} \ulaz{3}#
...
\includecodeLib{resenja/06_Liste/biblioteke/lista_iterativna/lista.h}{lista.h}
\includecodeLib{resenja/06_Liste/biblioteke/lista_iterativna/lista.c}{lista.c}
\includecodeLib{resenja/06_Liste/401_i_402/main_a.c}{main\_a.c}
\includecodeLib{resenja/06_Liste/401_i_402/main_b.c}{main\_b.c}
\includecodeLib{resenja/06_Liste/401_i_402/main_c.c}{main\_c.c}
\end{Answer}
...
\napomena{Koristiti iste \kckod{main} programe i test primere iz zadatka \ref{601}.}
\linkresenje{602}
\end{Exercise}
\begin{Answer}[ref=602]
\includecodeLib{resenja/06_Liste/biblioteke/lista_rekurzivna/lista.h}{lista.h}
\includecodeLib{resenja/06_Liste/biblioteke/lista_rekurzivna/lista.c}{lista.c}
\end{Answer}
...
\includecode{resenja/06_Liste/403.c}
\end{Answer}
...
Posle svakog unetog indeksa, program ispisuje poruku \kckod{da} ili \kckod{ne}, u zavisnosti od toga da li u listi postoji student sa unetim indeksom ili ne. 
...
\includecode{resenja/06_Liste/404.c}
\end{Answer}
...
\includecode{resenja/06_Liste/406.c}
\end{Answer}
...
ovih listi formira novu listu \kckod{L} koja sadrži naizmenično raspoređene čvorove 
listi \kckod{L1} i \kckod{L2}: prvi čvor iz \kckod{L1}, prvi čvor iz \kckod{L2}, drugi čvor \kckod{L1},
drugi čvor \kckod{L2}, itd. Ne formirati nove čvorove, već samo postojeće rasporediti u jednu listu. Prva lista se učitava iz datoteke čije se ime zadaje kao prvi argument komandne linije, a druga iz datoteke čije se ime zadaje kao 
...
\napomena{Iskoristiti testove 2 - 6 za zadatak \ref{606}.}
...
\includecode{resenja/06_Liste/408.c}
\end{Answer}
...
#\naslovIzlaz#
#\izlaz{Etikete nisu pravilno uparene}#
#\izlaz{(nadjena je etiketa </html>, a poslednja }#
#\izlaz{otvorena je <body>)}#
...
\includecodeLib{resenja/06_Liste/biblioteke/stek.h}{stek.h}
\includecodeLib{resenja/06_Liste/biblioteke/stek.c}{stek.c}
\includecodeLib{resenja/06_Liste/409.c}{main.c}
\end{Answer}
%=========================================================================
\begin{Exercise}[label=610]
Napisati program koji pomaže službeniku u radu na šalteru.
Službenik najpre evidentira sve korisničke \kckod{JMBG} brojeve (niske koje sadrže po $13$ karaktera) i zahteve (niska koja sadrži najviše $999$ karaktera). 
...
korisnik se stavlja na kraj reda, čime se obrada njegovog zahteva odlaže. Ukoliko odgovor nije \kckod{Da}, službenik obrađuje zahtev i podatke o korisniku dopisuje na kraj datoteke \kckod{izvestaj.txt}. Ova datoteka, za svaki obrađen zahtev, sadrži \kckod{JMBG} i zahtev usluženog korisnika.
...
\includecodeLib{resenja/06_Liste/biblioteke/red.h}{red.h}
\includecodeLib{resenja/06_Liste/biblioteke/red.c}{red.c}
\includecodeLib{resenja/06_Liste/410.c}{main.c}
\end{Answer}
...
\begin{enumerate}
 \item Napisati funkciju \kckod{void obrisi\_tekuci(Cvor ** adresa\_glave, Cvor ** adresa\_kraja, Cvor * tekuci)} koja briše čvor na koji pokazuje pokazivač \kckod{tekuci} iz liste čiji se pokazivač na čvor koji je glava liste nalazi na adresi \kckod{adresa\_glave} i poslednji čvor liste na adresi \kckod{adresa\_kraja}.
...
\napomena{Funkcije testirati koristeći test primere iz zadatka \ref{601}}
\linkresenje{611}
\end{Exercise}
\begin{Answer}[ref=611]
\includecodeLib{resenja/06_Liste/biblioteke/dvostruko_povezana_lista.h}{dvostruko\_povezana\_lista.h}
\includecodeLib{resenja/06_Liste/biblioteke/dvostruko_povezana_lista.c}{dvostruko\_povezana\_lista.c}
\includecodeLib{resenja/06_Liste/411/main_a.c}{main\_a.c}
\includecodeLib{resenja/06_Liste/411/main_b.c}{main\_b.c}
\includecodeLib{resenja/06_Liste/411/main_c.c}{main\_c.c}
\end{Answer}
...
%\napomena{Iskoristiti test 3 iz \ref{612}. zadatka.}

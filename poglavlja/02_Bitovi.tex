...
Napisati malu biblioteku za rad sa polinomima.
  ...
  \item Definisati strukturu \kckod{Polinom} koja predstavlja polinom
    ...
    koeficijenata. \uputstvo{redosled navođenja koeficijenata u nizu treba da bude takav da se na nultoj poziciji u nizu bude koeficijent uz slobodan član, na prvoj koeficijent uz prvi stepen, itd.}
...
Napisati program koji testira prethodno napisane funkcije tako što se najpre unosi polinom \argf{p} (stepen polinoma, a zatim i koeficijenti), ispisuje se polinom \argf{p} u odgovarajućem obliku. Zatim se od korisnika traži da unese tačka u kojoj se računa vrednost tog polinoma i ispisuje iztačunata vrednost zaokružena na dve decimale. Nakon toga unosi se polinom \argf{q}, a potom se ispisuju zbir i proizvod polinoma \argf{p} i \argf{q}. Na kraju se sa standardnog ulaza unosi broj \argf{n}, a potom se ispisuje \argf{n}-ti izvod polinoma \argf{p}.
...
\begin{Exercise}[label=202]
 Napisati funkciju koja broji bitove postavljene na \argf{1} u zapisu broja $x$, tako što se pomeranje vrši nad
\begin{enumerate}
\item maskom,
\item brojem $x$.
\end{enumerate} 
...
\includecode{resenja/02_Bitovi/202a.c}

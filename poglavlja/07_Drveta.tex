...
\begin{enumerate}
\item Definisati strukturu \kckod{Cvor} kojom se opisuje čvor binarnog pretraživačkog stabla koja sadrži ceo broj \kckod{b} i pokazivače \kckod{levo} i \kckod{desno} redom na levo i desno podstablo\footnote{U zadacima ove glave u kojima nije eksplicitno naglašen sadržaj čvorova stabla, podrazumevaće se ova struktura.}.

\item Napisati funkciju \kckod{Cvor* napravi\_cvor(int b)} koja alocira memoriju za novi čvor stabla i vrši njegovu inicijalizaciju zadatim celim brojem \argf{b}.

\item Napisati funkciju \kckod{void dodaj\_u\_stablo(Cvor** koren, int b)} koja u stablo na koje pokazuje argument \argf{koren} dodaje ceo broj \argf{b}.

\item Napisati funkciju \kckod{Cvor* pretrazi\_stablo(Cvor* koren, int b)} koja proverava da li se ceo broj \argf{b} nalazi u stablu sa korenom \argf{koren}. Funkcija vraća pokazivač na čvor stabla koji sadrži traženu vrednost ili NULL ukoliko takav čvor ne postoji.
...
\item Napisati funkciju \kckod{void obrisi\_element(Cvor** koren, int b)} koja briše čvor koji sadrži vrednost \argf{b} iz stabla na koje pokazuje argument \argf{koren}.
